<table>
    <thead>
        <tr>
            <th rowspan="2" style="font-weight:bold; text-align:center; background-color:#D9D9D9; color:#333333;">NO
            </th>
            <th rowspan="2" style="font-weight:bold; text-align:center; background-color:#D9D9D9; color:#333333;">NAMA
            </th>
            ...
            @foreach ($masterTagihans as $mt)
                ...
                @if (!$isSPP)
                    <th rowspan="2"
                        style="font-weight:bold; text-align:center; background-color:#D9D9D9; color:#333333;">
            ...
            @foreach ($masterTagihans as $mt)
                @php $isSPP = stripos($mt->nama_tagihan, 'spp') !== false; @endphp
                @if ($isSPP)
                    <th colspan="{{ count($bulanList) }}"
                        style="font-weight:bold; text-align:center; background-color:#FFF2CC; color:#333333;">
                        SPP
                    </th>
                @endif
            @endforeach

            <th rowspan="2" style="font-weight:bold; text-align:center; background-color:#4472C4; color:#ffffff;">TOTAL
            </th>
        </tr>
        <tr>
            @foreach ($masterTagihans as $mt)
    ...
    <tbody>
        @php
            ...
            foreach ($masterTagihans as $mt) {
                ...
                if ($isSPP) {
                    foreach ($bulanList as $bulan) {
                        $colTotals["spp_{$mt->id}_{$bulan}"] = 0;
                    }
                } else {
    ...
    <tfoot>
        <tr>
            <td colspan="2"
                style="font-weight:bold; text-align:center; background-color:#FFF2CC; color:#333333; font-size:9pt;">
                TOTAL
            </td>
            @foreach ($masterTagihans as $mt)
                @php $isSPP = stripos($mt->nama_tagihan, 'spp') !== false; @endphp
                @if (!$isSPP)
                    <td
                        style="text-align:right; font-weight:bold; background-color:#FFF2CC; color:#333333; font-size:9pt;">
